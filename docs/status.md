...
- Generic two-learner Bayesian `Mixture<A, B>` with posterior-odds updates from coding advantage; implemented on `feat/mixture-model` pending CI validation.
- Higher-is-better coding ratio (`baseline_cost / model_cost`), including the uniform-relative compression ratio, replaces bits-per-byte in the harness output on that branch.
