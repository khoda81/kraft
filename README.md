...
**Stage:** streaming byte harness implemented with uniform and adaptive unigram baselines. The first benchmark minimizes total coding cost on a local text stream; dataset results and model enumeration are still pending. See [current status](docs/status.md).
